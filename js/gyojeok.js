...
(function () {
  ...
  // 목회자 → 항존직 → 교회학교 부서 순. 부서(청년부·고등부…)는 직분은 아니지만
  // 학생·자녀의 '교회에서의 소속'을 이 칸에 적어 왔기에 같은 목록에 둔다.
  var ROLE_OPTS = ['담임목사', '원로목사', '담임사모', '원로사모',
    '장로', '원로장로', '안수집사', '권사', '은퇴권사', '집사', '권찰', '성도',
    '청년부', '고등부', '중등부', '초등부', '유년유치부', '유아부'];
  ...
  var GJ_NUMBERING_LIST = [
    ...
    // 아래는 첫 명부(교인기본정보 시트)에 없어 뒤늦게 붙인 분들 — 생년월일 순서와 무관하게 뒤로 이어 붙임
    [1055, '정보경'],   // 새가족
    [1056, '장한주']    // 신성호 배우자 — 원본 명부엔 '가족사항'에만 있어 누락됐었음
  ];
